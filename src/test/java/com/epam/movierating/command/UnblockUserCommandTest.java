...
public class UnblockUserCommandTest {
    ...
    public void setUp() {
        ...
        unblockUserCommand = new UnblockUserCommand(accountService);
    }
    ...
    public void testExecuteShouldReturnRedirectCommandResult() throws ServiceException {
        ...
        CommandResult commandResult = unblockUserCommand.execute(request);
        //then
    ...
    @Test
    public void testExecuteShouldUnblockUser() throws ServiceException {
        //given
        //when
        when(request.getParameter(Parameter.ID)).thenReturn(VALID_ID_PARAM);
        when(request.getServletContext()).thenReturn(servletContext);
        unblockUserCommand.execute(request);
        //then
        verify(accountService, times(1)).unblockUserById(VALID_ID);
    }
